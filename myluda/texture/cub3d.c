   ...
   // unsigned int colorr;
    ...
    while(j < start + wallStripHeight && j < 1000)
    {
        g_pourcent = (j - start) / wallStripHeight;
            ...
            if(check_opacity(textu0[(int)yOffset * 50 + (int)finder.x_offset]) != 0x000000)
            {
                g_data[1000 * j + i] = check_opacity(textu0[(int)yOffset * 50 + (int)finder.x_offset]);
                //printf("1\n");
            }
            if(swiitch == 0)
...
int ft_cast_ray(int index)
{
    ...
    sprite_indicator = 0;
    int stop = 1;
    sprites.wallHoriHit = 0;
    sprites.wallVertHit = 0;
    int check;
    wallHoriHit = 0;
    ray_Direction();
    ...
    while(nextHoriX >= 0 && nextHoriX < 650 && nextHoriY >= 0 && nextHoriY < 300)
    {
        ...
        if(sprite == 1)
        {
            sprites.wallHoriHit = 1;
            sprites.HorizwallHitX = nextHoriX;
            sprites.HorizwallHitY = nextHoriY;
        }
        if (check == 1)
        {
            wallHoriHit = 1;
    ...
    while(nextHoriX >= 0 && nextHoriX < 650 && nextHoriY >= 0 && nextHoriY < 300)
    {
        ...
        if(sprite == 1)
        {
            sprites.wallVertHit = 1;
            sprites.VertwallHitX = nextHoriX;
            sprites.VertwallHitY = nextHoriY;
...
// store the smallest of the distances
   ...
   // printf("%f\n",distance);
    ...
    {
        sprites.HorzHitDistance = (sprites.wallHoriHit) ? Distance_between_HorVer(X_player , Y_player, sprites.HorizwallHitX,sprites.HorizwallHitY) : MAX_INT;
       // mlx_pixel_put(mlx_ptr,win_ptr,sprites.HorizwallHitX,sprites.HorizwallHitY,0x000000);
        sprites.VertHitDistance = (sprites.wallVertHit) ? Distance_between_HorVer(X_player , Y_player, sprites.VertwallHitX,sprites.VertwallHitY) : MAX_INT;
       // printf("Hor ==> |%f|||||| Vert == >|%f| \n",sprites.HorzHitDistance, sprites.VertHitDistance);
        sprites.WallHitX = (sprites.HorzHitDistance) < (sprites.VertHitDistance) ? sprites.HorizwallHitX : sprites.VertwallHitX;
        sprites.WallHitY = (sprites.HorzHitDistance) < (sprites.VertHitDistance) ? sprites.HorizwallHitY : sprites.VertwallHitY;
        sprites.distance = (sprites.HorzHitDistance) < (sprites.VertHitDistance) ? sprites.HorzHitDistance : sprites.VertHitDistance;
        sprites.g_mhah = sprites.distance == sprites.HorzHitDistance ? 1 : 0;
        sprites.wasHitVertical = (sprites.VertHitDistance) < (sprites.HorzHitDistance);

        finder.angle_kbira = atan2(Y_player - sprites.WallHitY,  X_player - sprites.WallHitX);
        finder.angle_sghira = atan2(Y_player - center_y, X_player - center_x);
        finder.angle_wanted = finder.angle_kbira - finder.angle_sghira;

        finder.point_x = X_player + (center_x - X_player) * cos(finder.angle_wanted) - (center_y - Y_player) * sin(finder.angle_wanted);
        finder.point_y = Y_player + (center_x - X_player) * sin(finder.angle_wanted) + (center_y - Y_player) * cos(finder.angle_wanted);
        finder.angle_jdida = M_PI / 2 + finder.angle_sghira;
        finder.point_distance_x = center_x + (50 / 2) * cos(finder.angle_jdida);
        finder.point_distance_y = center_y + (50 / 2) * sin(finder.angle_jdida);
        finder.x_offset = sqrtf(powf(finder.point_distance_y - finder.point_y,2) + powf(finder.point_distance_x - finder.point_x,2));
        double diss = sqrtf(powf(finder.point_x - center_x,2) + powf(finder.point_y - center_y,2));
        if (diss > 25)
            finder.x_offset = 0;
        //mlx_pixel_put(mlx_ptr,win_ptr,finder.point_x,finder.point_y,0x0000ff);
        //printf("%f\n",deg(finder.angle_wanted));
       // printf("%f\n",finder.x_offset);
    }   
...
{
    correctWallDistance = distance_center * cos(rotation - rotationAngle);
   // printf("%f\n",sprites.distance);
   // printf("%f\n",distance);
   ...
   // alpha = 80 / correctWallDistance;
    distanceProjectionPlan = (1000 / 2) / tan(rad(45) / 2); //here delete cos
    wallStripHeight = (50 / correctWallDistance) * distanceProjectionPlan;
  //  finder.x_offset = sprites.g_mhah == 1 ? fmod(sprites.WallHitX,50) : fmod(sprites.WallHitY,50);
    ft_nizar_calcul();
        //Modification here 2
    //printf("%f\n",finder.x_offset);  
        if(swiitch == 1)
